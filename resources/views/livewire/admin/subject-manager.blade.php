@section('header', 'বিষয় (Subject) পরিচালনা')
...
<div class="space-y-6">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:px-6">
            <h3 class="text-lg leading-6 font-medium text-gray-900">বিষয়ের তালিকা</h3>
        </div>
    ...
    <div class="flex items-center justify-center space-x-1">
        ...
        @endif
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">বিষয়ের নাম (কোড)</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">শ্রেণী</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ধরন</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">পূর্ণমান / পাস নম্বর</th>
                    ...
                    @forelse($subjects as $subject)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $subject->schoolClass ? $subject->schoolClass->name : 'N/A' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 capitalize">
                                {{ $subject->subject_type_label }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $subject->full_marks }} / {{ $subject->pass_marks }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <button wire:click="edit({{ $subject->id }})" class="text-indigo-600 hover:text-indigo-900 mr-3">এডিট</button>
                                <button wire:click="delete({{ $subject->id }})" wire:confirm="আপনি কি নিশ্চিত?" class="text-red-600 hover:text-red-900">ডিলিট</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                                কোনো বিষয় পাওয়া যায়নি।
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                        ...
                        <div class="sm:flex sm:items-start">
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                                    {{ $isEditing ? 'বিষয় আপডেট করুন' : 'নতুন বিষয় যোগ করুন' }}
                                </h3>
                                <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label for="name" class="block text-sm font-medium text-gray-700">বিষয়ের নাম</label>
                                    ...
                                    <div>
                                        <label for="code" class="block text-sm font-medium text-gray-700">বিষয় কোড</label>
                                    ...
                                    <div>
                                        ...
                                        <select wire:model="class_id" id="class_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    ...
                                    <div>
                                        <label for="subject_type" class="block text-sm font-medium text-gray-700">ধরন</label>
                                        <select wire:model="subject_type" id="subject_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                            <option value="1">Core (আবশ্যিক)</option>
                                            <option value="2">Optional (ঐচ্ছিক)</option>
                                            <option value="3">Extra (অতিরিক্ত)</option>
                                        </select>
                                        @error('subject_type') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                    </div>

                                    <div>
                                        <label for="full_marks" class="block text-sm font-medium text-gray-700">পূর্ণমান (Full Marks)</label>
                                        <input type="number" wire:model="full_marks" id="full_marks" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        @error('full_marks') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                    </div>

                                    <div>
                                        <label for="pass_marks" class="block text-sm font-medium text-gray-700">পাস নম্বর (Pass Marks)</label>
                                        <input type="number" wire:model="pass_marks" id="pass_marks" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
